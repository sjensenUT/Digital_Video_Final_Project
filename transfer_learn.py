...
parser = argparse.ArgumentParser(description='Transfer Learning for Image De-Noising Model')
parser.add_argument('--init_model_path', type=str, default='./model/2.ckpt')
parser.add_argument('--checkpoint_path', type=str, default='./model/transfer.ckpt')
...
folderGT = os.path.join(image_dir, 'ground_truths')
folderNoisy = os.path.join(image_dir, 'noisy')
steps_per_epoch = len(os.listdir(folderGT))/batch_size

autoencoder.load_weights(init_model_path)
